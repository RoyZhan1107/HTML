...
const defaults = [
  ...
  {word:'participate', meaning:'參與（不及物 + in）', patterns: ['participate in ~','be willing to participate in ~'], fav:true},
];
...
function itemHTML(w, i){
  ...
  return `
    <div class="word" data-idx="${i}">
        ...
        <div>${patterns||'<span class="muted">（尚無句型）</span>'}</div>
        <div class="flex">
            <input class="pattern-input" placeholder="新增句型/例句…（Enter）" />
...
// --- 拼寫測驗 ---
let currentQ = null; // {answer, prompt}
...
function checkAnswer(){
    ...
    if(ok){
        resEl.innerHTML = `<span class="result-ok">✔ 正確！</span>`;
    }
    else if(near){
